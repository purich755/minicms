...
| `npm run migrate:sql` | Собрать все миграции в один файл для SQL Editor |
| `npm run tenant:create` | Подключить нового клиента: владелец, сайт, доступы |
| `npm run tenant:list` | Кто подключён и чей сайт выключен |
| `npm run tenant:disable -- <слаг>` | Выключить сайт (неоплата). Данные целы |
| `npm run tenant:enable -- <слаг>` | Включить обратно |
| `npm run backup` | Резервная копия всех сайтов в `backups/` |
| `npm run restore -- <папка>` | Вернуть из копии (без `--yes` только показывает) |
| `npm run lint` | ESLint |

**Схему меняют раньше кода, а не наоборот.** Код обращается к колонкам по
именам, и если выкатить его на базу без миграции, запрос отвалится, а человек
увидит «аккаунт не привязан к заведению» — сообщение, которое уводит совсем не
туда. Порядок: `npm run migrate:sql` → вставить в SQL Editor →
`npm run check:supabase` → и только потом деплой.

Работа с клиентами описана в [РУКОВОДСТВО.md](РУКОВОДСТВО.md), инструкция для
самого клиента — в [КЛИЕНТУ.md](КЛИЕНТУ.md).
...
Пошагово — в [DEPLOY.md](DEPLOY.md): репозиторий, Vercel, переменные, свой
домен, порядок внесения изменений и список граблей.

Коротко про адреса: сайт клиента открывается по пути `/flora` всегда, по
поддомену `flora.домен.рф` — после wildcard-домена, по своему домену
`flora-cafe.ru` — после записи в `tenants.custom_domain`.
